...
def generar_meditacion_personalizada(
    nombre_usuario: str,
    ...
    """
    Genera un audio de meditación personalizado de forma iterativa.
    """
    if not settings.ELEVENLABS_API_KEY or not settings.GOOGLE_API_KEY:
        raise ValueError(
            "Las claves API (ELEVENLABS_API_KEY, GOOGLE_API_KEY) deben estar "
...
IMPORTANTE: Responde únicamente con el texto del guion. No incluyas títulos, encabezados, ni explicaciones adicionales. Cada párrafo debe estar separado por un doble salto de línea.
        ...
        try:
            ...
            for pause_marker, duration_ms in PAUSE_DURATIONS.items():
                segment_pause_duration += segment_text.count(pause_marker) * duration_ms
    ...
    return os.path.abspath(ruta_archivo_audio), os.path.abspath(ruta_script_txt)
